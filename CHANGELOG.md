...
### Bug Fixes
- **Tooltip no longer blinks on dashboards with a shared crosshair or tooltip**: With the dashboard time-range option "Shared crosshair"/"Shared tooltip" on, the panel traded cursor-sync events with itself and with the other charts on the dashboard, resetting the uPlot cursor about ten times a second. The tooltip was torn down roughly 100ms after every time it opened, so it flickered whenever a point was hovered; the worst case was moving the pointer over a numeric X-axis chart, whose sample numbers the time-based charts read as timestamps, which left every panel on the dashboard flickering. Cursor sync between charts now goes through uPlot directly, the way current Grafana panels do, so the tooltip stays put — and the shared crosshair now also travels from an SPC chart to the other panels, which it previously did not.
- **Feature queries are selectable again, and behave like reference data**: The "Feature queries" option, which marks queries as reference-only, was documented but never registered in the panel editor — it could only be set by hand-editing the panel JSON. It now appears in the **SPC** section. A feature query is also properly consumed as a reference now: it is drawn only through the control lines that read it, in the color configured on the line, instead of being plotted as an extra series with its own palette color and points, and the chart type transform no longer runs over specification values. ([#65](https://github.com/KensoBI/spc-chart/issues/65))
- **Numeric X-axis works whatever order the queries come back in**: The X field was located by looking up its name in the *first* query only, and the resulting column index was then applied to every other query by position. A panel whose first query did not carry the X column — a specification-limit lookup, say — silently fell back to a time axis and charted the X column as though it were a measurement; and queries that returned their columns in different orders had the wrong column treated as the X axis. The X field is now matched by name in each query independently, so any query may supply it and column order no longer matters.
